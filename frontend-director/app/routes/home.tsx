...
export default function Dashboard() {
  ...
  const successRate = ov && ov.totalRecords > 0 ? Math.round((ov.doneRecords / ov.totalRecords) * 100) : 0;

  const trendData = useMemo(
    () => (stats?.trend ?? []).map((t) => ({ day: formatDate(t.date), total: t.total, done: t.done, failed: t.failed, missed: t.missed })),
    [stats],
  );
  ...
  const ownershipData = useMemo(
    () => stats?.ownership
      ? [
          { name: "Привязано", value: stats.ownership.assigned, fill: "#3b82f6" },
          { name: "Без владельца", value: stats.ownership.unassigned, fill: "#f97316" },
        ]
      : [],
    [stats],
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        {isPending
          ...
          : (
            <>
              <StatCard title="Всего записей" value={ov?.totalRecords ?? 0} sub="за период" icon={FileAudio} onClick={() => navigate("/calls")} />
              <StatCard title="Выполнено" value={ov?.doneRecords ?? 0} sub={`${successRate}% от всех`} icon={TrendingUp} />
              <StatCard title="Средний балл" value={formatPercent(ov?.avgQualityScore ?? null)} sub="quality score" icon={Activity} />
              <StatCard title="Пропущено" value={ov?.missedRecords ?? 0} sub="нет аудио / пропущено" icon={PhoneMissed} />
              <StatCard title="Ср. длит." value={formatDuration(ov?.avgTalkDurationSec ?? null)} sub="разговора" icon={Clock} />
              <StatCard title="Менеджеров" value={ov?.totalManagers ?? 0} sub="нажмите для просмотра" icon={Users} onClick={() => setManagersOpen(true)} />
            </>
          )}
      </div>
        ...
        {isPending ? (
          <>
            ...
            <Card>
              ...
              <CardContent><Skeleton className="h-[160px] w-full rounded-lg" /></CardContent>
            </Card>
          </>
        ) : (
          <>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={trendData}>
                    ...
                    <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} formatter={(v: number, name: string) => [v, { total: "Всего", done: "Выполнено", failed: "Ошибки", missed: "Пропущено" }[name] ?? name]} />
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Привязка записей</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col items-center">
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart>
                    <Pie data={ownershipData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" paddingAngle={3}>
                ...
                <div className="mt-2 flex flex-col gap-1 self-start text-xs text-neutral-600">
                  ...
                  {stats?.ownership.unassignedMango != null && stats.ownership.unassignedMango > 0 && (
                    <span className="mt-1 text-neutral-400">в т.ч. Mango без маппинга: {stats.ownership.unassignedMango}</span>
                  )}
                </div>
              </CardContent>
            </Card>
      ...
      <div className="mt-4">
        {isPending ? (
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">Статистика по менеджерам</CardTitle>
            </CardHeader>
              ...
              <Table>
                ...
                <TableBody>
                  {(stats?.byAgent ?? []).length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="py-10 text-center text-sm text-neutral-400">Нет данных</TableCell>
                    </TableRow>
                  ) : (
                    (stats?.byAgent ?? []).map((row) => (
                      ...
                      >
                        ...
                        <TableCell className="text-green-600 dark:text-green-400">{row.done}</TableCell>
                        <TableCell className="text-red-600 dark:text-red-400">{row.failed}</TableCell>
                        <TableCell className="text-orange-600 dark:text-orange-400">{row.missed}</TableCell>
                        <TableCell className="tabular-nums">{formatDuration(row.avgTalkDurationSec)}</TableCell>
                        <TableCell>{formatPercent(row.avgQualityScore)}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
