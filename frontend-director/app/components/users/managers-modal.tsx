...
export function ManagersModal({ open, onClose, currentUserId }: ManagersModalProps) {
            ...
            <DialogContent className="w-[min(440px,calc(100%-2rem))] max-w-none p-0 gap-0 [&>button:last-child]:hidden">
                <DialogHeader className="border-b border-neutral-200 dark:border-neutral-800 px-5 py-4">
                    <DialogTitle className="text-base font-semibold">
                        Менеджеры
                        <span className="ml-2 text-sm font-normal text-neutral-400">{managers.length}</span>
                    </DialogTitle>
                </DialogHeader>
                <div className="max-h-[60vh] overflow-y-auto p-2">
                    {managers.length === 0 ? (
                        ...
                        managers.map((u) => (
                            <ManagerRow key={u.id} user={u} onClick={() => handleClick(u.id)} />
                        ))
                    )}
                </div>
            </DialogContent>
