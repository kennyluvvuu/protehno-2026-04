...
export function SearchModal({ open, onOpenChange }: SearchModalProps): React.ReactElement {
    ...
    const results = useMemo(() => {
        ...
        if (!q) return []
        return records.reduce<{ record: Record; matches: string[] }[]>((acc, record) => {
            const matches = matchRecord(record, q)
            if (matches.length > 0) acc.push({ record, matches })
            return acc
        }, []).slice(0, 8)
    }, [query, records])
            ...
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="h-[110vh] max-h-[600px] w-[min(620px,calc(100%-2rem))] max-w-none grid-rows-[auto_1fr] overflow-hidden p-0 [&>button:last-child]:hidden">
                    ...
                    <div className="overflow-y-auto p-3">
                        {!hasQuery ? (
                            <p className="py-20 text-center text-sm text-muted-foreground">Начните вводить текст</p>
                        ) : results.length === 0 ? (
                            <p className="py-20 text-center text-sm text-muted-foreground">Ничего не найдено</p>
                        ) : (
                                ...
                                {results.map(({ record, matches }) => (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="truncate text-sm font-medium">{record.title ?? `Звонок #${record.id}`}</p>
                                            <p className="truncate text-xs text-muted-foreground">
                                                {[record.callTo, record.startedAt ? new Date(record.startedAt).toLocaleDateString("ru-RU") : null].filter(Boolean).join(" · ")}
                                            </p>
                                            <div className="mt-1.5 flex flex-wrap gap-1">
                                                {matches.slice(0, 3).map((label) => (
                                                    <Badge key={label} variant="outline" className={cn("text-[10px]")}>{label}</Badge>
                                                ))}
                                            </div>
                                        </div>
                                    </button>
