...
const FUSE_KEYS: { name: keyof Record; weight: number }[] = [
    { name: "title", weight: 0.4 },
    { name: "callTo", weight: 0.35 },
    { name: "summary", weight: 0.15 },
    { name: "transcription", weight: 0.1 },
]

const FIELD_LABELS: Partial<{ [K in keyof Record]: string }> = {
    title: "Название",
    callTo: "Контрагент",
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {results.map(({ item: record, matches }) => {
                                    const matchedFields = [...new Set(
                                        (matches ?? [])
                                            .map((m) => FIELD_LABELS[m.key as keyof Record])
                                            .filter(Boolean)
                                    )] as string[]
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="truncate text-xs text-muted-foreground">
                                                    {[record.callTo, formatRecordDate(record)]
                                                        .filter(Boolean)
                                                        .join(" · ")}
                                                </p>
