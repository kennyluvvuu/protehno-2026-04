...
type SearchableRecord = Record & { searchText: string };

const FUSE_KEYS: { name: keyof SearchableRecord; weight: number }[] = [
  { name: "title", weight: 0.3 },
  { name: "callTo", weight: 0.25 },
  { name: "summary", weight: 0.15 },
  { name: "transcription", weight: 0.12 },
  { name: "callerNumber", weight: 0.08 },
  { name: "calleeNumber", weight: 0.07 },
  { name: "directionKind", weight: 0.02 },
  { name: "source", weight: 0.01 },
];
...
function StatusBadge({ status }: { status: RecordStatus }) {
  ...
  if (status === "not_applicable") {
    return (
      <Badge variant="outline" className="border-neutral-200 bg-neutral-50 text-neutral-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-400">
        Нет аудио
      </Badge>
    );
  }
  if (status === "queued") {
    return (
      <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-700 dark:bg-blue-950/35 dark:text-blue-300">
        В очереди
      </Badge>
    );
  }
  if (status === "uploaded") {
    return (
      <Badge variant="outline" className="border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-700 dark:bg-sky-950/35 dark:text-sky-300">
        Загружено
      </Badge>
    );
  }
...
export default function Calls() {
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortField === "callTo")
        return dir * getDisplayCounterparty(a).localeCompare(getDisplayCounterparty(b), "ru");
      if (sortField === "durationSec")
        return dir * ((getDisplayDuration(a) ?? 0) - (getDisplayDuration(b) ?? 0));
      return dir * (getRecordDate(a) ?? "").localeCompare(getRecordDate(b) ?? "");
    });

    return list;
  }, [searchableRecords, search, sortField, sortDir, callKindFilter, directionFilter, processingFilter, dateFrom, dateTo, fuse]);
  ...
  return (
    <div>
      <PageHeader
        title="Мои звонки"
        description="Записи ваших разговоров"
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="relative min-w-48 flex-1">
          ...
          <Input
            placeholder="Поиск по звонкам, контрагенту или тексту…"
            value={search}
        ...
        <div className="flex items-center overflow-hidden rounded-md border border-input bg-background">
          <div className="relative flex h-9 items-center gap-1.5 border-r border-input px-3">
            ...
            <button
              ...
              className="cursor-pointer min-w-14 select-none text-left text-xs"
            >
              {dateFrom ? (
                formatShortDate(dateFrom)
              ) : (
                <span className="text-neutral-300 dark:text-neutral-600">дд.мм.гг</span>
              )}
            </button>
            <input
              ref={dateFromRef}
          ...
          {isDateActive && (
            <button
              ...
              title="Сбросить"
              className="cursor-pointer flex h-9 w-8 items-center justify-center border-l border-input text-neutral-400 transition-colors hover:text-neutral-700 dark:hover:text-neutral-200"
            >
              <X className="size-3" />
            </button>
          )}
