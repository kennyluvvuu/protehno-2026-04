import { Activity, ArrowRight, ClipboardList, FileAudio, List, TrendingUp } from "lucide-react"
    ...
    const pendingTasks = useMemo(() => {
        return records.flatMap((record) =>
            (record.checkboxes?.tasks ?? [])
                .filter((t) => !t.checked)
                .map((t) => ({ text: t.label, recordTitle: record.title ?? `Звонок #${record.id}`, callTo: record.callTo }))
        ).slice(0, 5)
    }, [records])
        ...
        <div>
            ...
            {!isPending && total === 0 && (
                <div className="mt-8 flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-200 px-6 py-16 text-center dark:border-neutral-800">
                    ...
                    <p className="mt-1 max-w-xs text-xs text-neutral-500">Загрузите первый звонок через раздел «Загрузить»</p>
                </div>
            )}
                    ...
                    <Card>
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Button asChild variant="ghost" size="sm" className="gap-1 text-xs text-neutral-500 hover:text-neutral-800 dark:hover:text-neutral-200">
                                    <Link to="/tasks">
                                        Все задачи
